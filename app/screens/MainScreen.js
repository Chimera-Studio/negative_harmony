...
function showScales() {
	...
	if (selectedScaleValSaved == "major") {
		...
		for (let i = 0; i < majorPentatonicScale.length; i++) {
			...
			negativeScale.push(
				cloneNegativeScale[majorPentatonicScale[pattern]]
			);
			pattern++;
		}
		...
		for (let i = 0; i < minorPentatonicScale.length; i++) {
			...
			negativeScale.push(
				cloneNegativeScale[minorPentatonicScale[pattern]]
			);
			pattern++;
		}
	...
	return (
		<View style={styles.screenWrapper}>
			...
			<View style={styles.selectWrapper}>
				{legendStatus ? (
					<Animated.View
						style={[styles.legendContainer, { opacity }]}
					>
						...
						>
							{scaleList.map((scale, index) => (
								<TouchableOpacity
									...
									key={scale.value}
									onPress={() =>
										selectedScale(scale.name, scale.value)
									}
								>
		...
		<View style={styles.chordWrapper}>
			<View style={styles.positiveChord}>
				<View style={styles.positiveChordNameWrapper}>
					<Text style={styles.positiveChordTonic}>
						{selectedTonic}
					</Text>
					<Text style={styles.positiveChordName}>
						{selectedChordVal}
					</Text>
				</View>
			...
			<View style={styles.negativeChord}>
				<View style={styles.negativeChordNameWrapper}>
					<Text style={styles.negativeChordTonic}>
						{negativeTonic}
					</Text>
					<Text style={styles.negativeChordName}>
						{negativeChordVal}
					</Text>
				</View>
			...
			<View style={styles.chordsWrapper} onLayout={getDimentions}>
				{tonicSliderOpen ? (
					<View style={styles.scrollChords}>
						<Text style={styles.scrollChordsExpText}>
							Choose a tonic:
						</Text>
					...
					>
						<Text style={styles.rewardedOpenText}>
							Unlock negative chords
						</Text>
					</TouchableHighlight>
				)}
			</View>
			...
			<View style={styles.discTextWrapper}>
				<ScrollView showsVerticalScrollIndicator={false}>
					<Text style={styles.discText}>
						...
						{"\n"}(Non-Diatonic refers to any notes or chords that
						are not native to the key){"\n"}
						{"\n"}This does not mean that the chord cannot be used
						in that key/scale but that its sound is more complex and
						will not fit with the other chords as easily.
					</Text>
					<Text style={styles.discSubTitle}>Disclaimer</Text>
					<Text style={styles.discText}>
						The negative chord names may not always be exactly right
						because of the way the chord naming system works. {"\n"}
						{"\n"}In practise the chord name comes from the
						tonic/root note followed by the structure/pattern
						(major, minor, suspended...) this rule is broken when
						looking at negative chords where the tonic/root is
						actually the last note played going from left to right
						on the piano. {"\n"}
						{"\n"}Because of this, the app looks if the negative
						chord pattern has a match when mirrored and compared to
						the regular one. If so it determines the chord name by
						taking the "negative" tonic/root note and displaying the
						chord name. {"\n"}
						{"\n"}Though the chord name may vary depending on what
						you consider to be the tonic/root note the notes
						displayed in the negative chords are always correct.
					</Text>
					<Text style={styles.discContactTitle}>Contact</Text>
					<Text style={styles.discText}>
						If you find any bugs incorrect representations of the
						notes or chords, please contact us at:{"\n"}
...
function MainScreen() {
	...
	return (
		<SafeAreaView style={styles.container}>
			<StatusBar hidden />
			{rewardedOpen || disclamerOpen ? null : <Background />}

			{!pressAlert ? null : (
				<Animated.View style={[styles.alert, showAlert]}>
					<Text style={styles.alertText}>
						You need to tap on a field!
					</Text>
				</Animated.View>
			)}
